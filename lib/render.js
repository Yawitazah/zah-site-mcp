...
function formNote(status, ctx = {}) {
  ...
  switch (status) {
    case 'zah-crm': return 'This form sends to ZAH CRM: every submission becomes a lead in the client\'s CRM.';
    case 'zah-crm-off': return `ZAH CRM is not connected on this site yet, so this form is inert until it is. That is the default place for forms on a ZAH site (leads, follow-up, invoicing in one place). Ask the client: would you like this form connected to ZAH CRM? If yes, send them to Zah (${contact}) to switch it on; nothing needs to be built on the site. If they would rather use their own outside service, point the form's action at that service's https address instead.`;
    case 'external': return 'This form posts to the client\'s own outside service. Nothing is stored on this site. If they would prefer it in ZAH CRM, the default for ZAH sites, ask Zah to connect it.';
