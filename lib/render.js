...
const DEFAULT_EDITABLE = ['h1', 'h2', 'h3', 'h4', 'h5', 'p', 'li', 'blockquote', 'figcaption', 'a', 'button', 'img', 'video', 'span', 'label', 'strong', 'b', 'small', 'em', 'dt', 'dd'];
const STRUCTURAL = ['header', 'nav', 'main', 'section', 'article', 'aside', 'footer', 'figure', 'ul', 'ol', 'div', 'table'];
const DEFAULT_CHROME = ['script:not([data-zs-keep])', 'noscript:not([data-zs-keep])', '#edToggle', '#edBar', '#edBubble', '#edEl', '.edbub', '.zp-modal', '[data-zs-chrome]', '[data-zs-credit]'];

/* THE HOUSE CREDIT. Every page a ZAH site serves links back to
   zahbrandsolutions.com. Zah, 2026-09-08: "any product that we built, I don't
   want that to ever disappear... even if they changed the words, it always
   should link to Zah Brand Solutions." A credit marked data-zs-credit is
   chrome, so neither the editor nor the AI can key it. And whatever the page
   ends up as, render() checks for a link to the house and puts one back if
   it is gone. mount({ credit: false }) is the only way off, and that is
   Zah's call, not the client's. */
const CREDIT_HOST = 'zahbrandsolutions.com';
const CREDIT_HTML = '<div data-zs-credit style="text-align:center;font:12px/1.6 system-ui,Arial,sans-serif;color:#6b7280;padding:14px 16px;">Site by <a href="https://zahbrandsolutions.com" target="_blank" rel="noopener" style="color:inherit;text-decoration:underline;">ZAH Brand Solutions</a></div>';
function ensureCredit($, opts) {
  if (opts.credit === false) return;
  const linked = $('a[href*="' + CREDIT_HOST + '"]').filter((_, a) => !$(a).parents('[hidden]').length && ($(a).attr('style') || '').indexOf('display:none') < 0);
  if (linked.length) return;
  $('body').append(CREDIT_HTML);
}
const ATTR_WHITELIST = ['href', 'src', 'alt', 'title', 'target', 'poster', 'width', 'height', 'loading', 'style'];
...
function keyBody($, opts, mode) {
  ...
  const body = $('body');
  const counters = {};
  const occupied = new Set(body.find('[data-zs]').map((_, el) => $(el).attr('data-zs')).get());
  const nextKey = (tag) => {
    if (mode === 'random') return randomKey();
    let n = counters[tag] || 0; while (occupied.has(`${tag}:${n}`)) n++; counters[tag] = n + 1; occupied.add(`${tag}:${n}`); return `${tag}:${n}`;
  };
  const elements = [];
  const outline = [];
  ...
  // Reserve the original outer-element keys before adding newly editable
  // nested labels. Existing consumers may have overlays keyed as a:3/p:2;
  // expanding the editor must not move those edits to a different element.
  const legacy = ['h1', 'h2', 'h3', 'h4', 'h5', 'p', 'li', 'blockquote', 'figcaption', 'a', 'button', 'img', 'video', 'span.badge', 'label'].join(', ');
  body.find(legacy).each((_, el) => {
    const node = $(el);
    if (isChrome($, el, chromeSel) || !node.is(editable)) return;
    if (node.parents(legacy).length && !node.attr('data-zs')) return;
    if (!node.attr('data-zs')) node.attr('data-zs', nextKey(el.tagName.toLowerCase()));
  });

  body.find(editable).each((_, el) => {
    const $el = $(el);
    if (isChrome($, el, chromeSel)) return;
    // Nested spans and links have their own keys, so every label is reachable.
    const tag = el.tagName.toLowerCase();
    if (tag === 'span' && !/[\p{L}\p{N}]/u.test($el.text())) return;
    let key = $el.attr('data-zs');
    if (!key) { key = nextKey('inline:' + tag); $el.attr('data-zs', key); }
    elements.push({
      key, tag,
...
function render(fileHtml, opts, overlay, page) {
  ...
  // Share cards and search snippets follow the same three fields.
  if (meta && (meta.title || meta.description !== undefined || meta.image)) {
    const setMeta = (sel, attrs) => { let el = $(sel); if (!el.length) { $('head').append('<meta>'); el = $('head meta').last(); } el.attr(attrs); };
    if (meta.title) setMeta('meta[property="og:title"]', { property: 'og:title', content: meta.title });
    if (meta.description !== undefined) setMeta('meta[property="og:description"]', { property: 'og:description', content: meta.description });
    if (meta.image) {
  ...
  keyBody($, opts, 'positional');
  applyEdits($, overlay.edits && overlay.edits[page]);
  $('body [data-zs-setting]').each((_, el) => {
    const node = $(el), key = node.attr('data-zs-setting');
    const value = overlay.settings && overlay.settings[key];
    if (value === undefined) return;
    if (node.attr('data-zs-setting-text') !== undefined) {
      const prefix = node.attr('data-zs-setting-prefix') || '';
      setText($, node, prefix + value);
    }
    const protocol = node.attr('data-zs-setting-href');
    if (['tel', 'sms', 'mailto'].includes(protocol)) {
      let target = value;
      if (protocol !== 'mailto') {
        target = String(value).replace(/\D/g, '');
        if (target.length === 10) target = '1' + target;
        target = '+' + target;
      }
      node.attr('href', protocol + ':' + target);
    }
  });
  ...
  const { elements, outline } = keyBody($, opts, 'positional');
  ensureCredit($, opts);
