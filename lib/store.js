   ...
   Shape:
     {
       ...
       settings:  { bookingUrl: "https://...", phone: "..." }  // site-wide values the host declares
     }
   ========================================================= */
const fs = require('fs');
const path = require('path');

const EMPTY = () => ({ version: 0, updatedAt: null, updatedBy: null, snapshots: {}, edits: {}, settings: {} });
...
class Store {
  ...
  /** Merge site settings. Validation is the mount's job; this just stores. */
  ...
  revert(version, by) {
    ...
    return this.write({ snapshots: prev.snapshots || {}, edits: prev.edits || {}, settings: prev.settings || {} }, by || 'revert');
  }
}
