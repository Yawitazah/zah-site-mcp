...
function buildServer(site) {
  ...
  server.registerTool('get_site', {
    ...
    pages: ops.listPages(), settings: ops.settingsSchema, usage: ops.assets.usage(),
    how: [
      ...
      'Forms and data collection are welcome when they post to the client\'s own outside service (Formspree, Google Forms, Airtable, their CRM, a Calendly embed). This server keeps no data for them, so a form pointed at this site is made inert. Storage stops at the quota (get_usage). Integrating with Zah\'s platform is Zah\'s paid work: ask him.',
    ],
  }));
