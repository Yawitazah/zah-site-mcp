...
function buildServer(site) {
  ...
  server.registerTool('set_page_meta', { title: 'Page SEO: title, description, share image', description: 'Set a page\'s browser title, search description and the image shown when the link is shared (og:image). Works on every page, built or created. Empty string clears a field.', inputSchema: { page: pageArg, title: z.string().max(200).optional(), description: z.string().max(300).optional(), image: z.string().max(500).optional().describe('Absolute https URL or a site path like /zah-site/assets/hero.jpg') } },
    async ({ page, title, description, image }) => {
      const p = pageOr(page); if (!p) return noPage(page);
      if (image && !/^(https:\/\/|\/)/.test(image)) return fail('image must be an https URL or a path starting with /');
      return out(ops.setPageMeta(p, { title, description, image }, 'mcp'));
    });
  server.registerTool('get_page_meta', { title: 'Page SEO as set', description: 'The title, description and share image currently set for a page (blank means the build\'s own).', inputSchema: { page: pageArg } },
    async ({ page }) => { const p = pageOr(page); if (!p) return noPage(page); const m = (ops.store.read().pages || {})[p.path] || {}; return text({ page: p.path, title: m.title || '', description: m.description || '', image: m.image || '' }); });
