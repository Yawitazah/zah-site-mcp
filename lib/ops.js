...
class SiteOps {
  ...
  setHtml(p, key, html, by) {
    ...
    return this.withDom(p, ($) => {
      ...
      if (!$el) return { error: `No element "${key}" on ${p.path}.` };
      if ($el.is('body')) { $el.html(R.sanitize(html, { host: this.host })); return { replaced: 'body' }; }
      const frag = R.keyFragment(R.sanitize(html, { host: this.host }), this.opts);
      $el.replaceWith(frag);
      return { replaced: key };
    }, by);
  }
  ...
  insertHtml(p, html, position, targetKey, by) {
    ...
    return this.withDom(p, ($) => {
      ...
      return { inserted: keys, position, target: targetKey || 'body' };
    }, by);
  }
  ...
  createPage(pagePath, meta, from, html, by) {
    ...
    if (html !== undefined && html !== null) {
      ...
      const clean = R.sanitize(html, { host: this.host });
    ...
    return { ok: true, version: state.version, path: np, url: np };
  }
